...
支持 gemini-3.1-flash-image-preview 等多种模型
用于微信公众号文章配图生成 - 高级版
...
class YunwuImageGenerator:
    ...
    def generate_impactful_cover(
        ...
        # 检查是否是OpenClaw相关文章
        is_openclaw_article = brands and any(b.lower() in ['openclaw', 'clawdbot', 'moltbot'] for b in brands)
        
        if is_openclaw_article:
            # OpenClaw文章：必须包含赛博小龙虾元素
            brand_elements = """
CORE VISUAL - CYBER LOBSTER MASCOT (CRITICAL - OpenClaw Brand):
- Central focal point: A stylized cyberpunk lobster/crayfish icon (OpenClaw's brand mascot)
- The lobster should be rendered in a modern, tech-inspired style
- Use neon blue and purple gradients for the lobster shell (#7B68EE to #00BFFF)
- Add circuit patterns or digital grid overlays on the lobster
- The lobster should appear powerful and futuristic, representing AI evolution
- Position prominently in center or right side of image
- The lobster represents the "claw" in OpenClaw - powerful and precise

VISUAL STYLE:
- Dramatic lighting with blue and purple neon glows
- Bold color scheme: Deep blues (#1E3A5F), Electric purples (#7B68EE), Neon cyan accents
- Tech-inspired background with circuit patterns, data streams
"""
        elif include_brand_logos and brands:
...
- Connections: Flows to next node""")
        ...
        if is_openclaw_article:
            ...
            brand_section = """
CORE VISUAL - CYBER LOBSTER MASCOT (CRITICAL - OpenClaw Brand):
- Include a stylized cyberpunk lobster/crayfish icon (OpenClaw's brand mascot)
- The lobster should be rendered in a modern, tech-inspired style
- Use neon blue and purple gradients for the lobster shell (#7B68EE to #00BFFF)
- Add circuit patterns or digital grid overlays on the lobster
- The lobster should appear powerful and futuristic
- Position as a visual anchor or decorative element in the infographic
- The lobster represents the "claw" in OpenClaw - powerful and precise

VISUAL STYLE:
- Dramatic lighting with blue and purple neon glows
- Bold color scheme: Deep blues (#1E3A5F), Electric purples (#7B68EE), Neon cyan accents
- Tech-inspired background elements with circuit patterns"""
...
def generate_article_images(
    ...
    关键改进：
    - 默认使用 gemini-3.1-flash-image-preview 模型，成本更低
    - 所有提示词使用中文，强制输出简体中文
