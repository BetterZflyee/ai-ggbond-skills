...
def load_all_env() -> Dict[str, str]:
    ...
    优先级（高到低）：
    1. 系统环境变量 (os.environ)
    2. 全局项目配置: F:\AI Workstation\AI\Super_OPC\.ai-ggbond-skills\.env (类似 .ai-ggbond-skills)
    ...
    # 加载全局项目配置（类似 .ai-ggbond-skills，最高优先级配置文件）
...
# gpt-image-2 使用 OpenAI Images API (/v1/images/generations)，中文渲染效果最佳
