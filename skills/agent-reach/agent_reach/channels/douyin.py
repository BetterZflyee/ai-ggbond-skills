...
class DouyinChannel(Channel):
    ...
    def check(self, config=None):
        ...
        try:
            r = subprocess.run(
                ["mcporter", "list"], capture_output=True, text=True, timeout=10, encoding='utf-8', errors='replace', shell=True
            )
        ...
        try:
            r = subprocess.run(
                ...
                capture_output=True, text=True, timeout=15, encoding='utf-8', errors='replace', shell=True
            )
            if r.returncode == 0:
