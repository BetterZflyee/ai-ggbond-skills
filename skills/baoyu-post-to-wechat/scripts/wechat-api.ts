...
function extractHtmlContent(htmlPath: string): string {
  ...
  const headStyles = styleMatches.map((item) => item[0].trim()).join("\n");

  const outputMatch = html.match(/<div[^>]*\sid=["']output["'][^>]*>[\s\S]*?<\/div>/i);
  const bodyMatch = html.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
  const extracted = outputMatch ? outputMatch[0].trim() : (bodyMatch ? bodyMatch[1]!.trim() : html);
