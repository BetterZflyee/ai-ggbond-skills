...
class ArticleManager:
    ...
    def generate_images(
        ...
        try:
            ...
            for i, spec in enumerate(image_specs, 1):
                ...
                try:
                    quality = "hd" if img_type == "cover" and model in ["gemini-3.1-flash-image-preview", "qwen-image-max", "dall-e-3", "gpt-image-1"] else "standard"
    ...
    def process_file(self, input_path, generate_images: bool = False, image_model: str = "gemini-3.1-flash-image-preview"):
        """
        处理输入的Markdown文件
    ...
    def process_content(self, markdown_content, generate_images: bool = False, image_model: str = "gemini-3.1-flash-image-preview"):
        """
        处理Markdown内容
...
示例:
  ...
  python article_manager.py -c "Markdown内容..." --generate-images --model flux-1.1-pro
    ...
    parser.add_argument('-m', '--model', default='gemini-3.1-flash-image-preview',
                        help='图片生成模型 (默认: gemini-3.1-flash-image-preview)')
