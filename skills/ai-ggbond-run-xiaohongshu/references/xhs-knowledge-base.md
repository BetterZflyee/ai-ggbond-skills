# XHS Knowledge Base（Memory 联动版）

本文件定义「知识库」核心能力：把每次分析、选题、发布、回复、复盘中的有效信息，沉淀成可检索、可复用、可追踪的结构化记录。

**增强：高价值结论同步写入 Hermes Memory，实现跨会话检索。**

目标不是存一堆日志，而是让后续决策能快速回答三个问题：
- 这类内容之前怎么做的
- 哪些动作有效、哪些动作失败
- 下一次应该直接复用什么
- **（NEW）跨会话后我还能记得什么（Memory 联动）**
...
## 9. Hermes Memory 联动（NEW）

高价值结论除了写入本地 markdown 知识库，还应同步写入 Hermes Memory 以实现跨会话检索。

### 9.1 Memory 写入时机

| 结论类型 | Memory 格式 | 示例 |
|---------|-----------|------|
| 已验证的 Pattern | `[XHS-Pattern] <模式描述> — 来源: <URL>` | `[XHS-Pattern] 标题"为什么___"句式互动率高 — 来源: xhslink.com/xxx` |
| 账号定位更新 | `[XHS-Positioning] <定位要素>` | `[XHS-Positioning] 内容支柱调整为: AI落地+超级个体+行业判断` |
| 避坑经验 | `[XHS-Pitfall] <避坑描述>` | `[XHS-Pitfall] 发布避开周一上午，受众活跃在周二三四晚` |
| Persona 偏好 | `[XHS-Persona] <风格偏好>` | `[XHS-Persona] 评论回复偏好: 专业有温度，不超过3句` |

### 9.2 双写规则

- 本地 knowledge-base markdown：完整记录（含证据、URL、截图路径）
- Hermes Memory：精简摘要（一句话结论 + 来源 + 可复用场景）
- Memory 内容保持紧凑，避免占用过多 token 配额

### 9.3 Memory 检索

下次任务启动时，Hermes Memory 会自动注入当前会话。
skill 在启动时会扫描 `[XHS-*]` 前缀的记忆条目，用于快速恢复上下文。
