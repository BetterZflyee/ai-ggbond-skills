...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.ai-ggbond-skills', '.env');
  const userEnv = path.join(os.homedir(), '.ai-ggbond-skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== aiggbond-post-to-wechat: Permission & Environment Check ===\n');
