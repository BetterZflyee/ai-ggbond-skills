...
export function loadWechatExtendConfig(): WechatExtendConfig {
  const paths = [
    path.join(process.cwd(), ".ai-ggbond-skills", "aiggbond-post-to-wechat", "EXTEND.md"),
    path.join(
      process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config"),
      "ai-ggbond-skills", "aiggbond-post-to-wechat", "EXTEND.md"
    ),
    path.join(os.homedir(), ".ai-ggbond-skills", "aiggbond-post-to-wechat", "EXTEND.md"),
  ];
...
function resolveCredentialSource(
  ...
  throw new Error(
    ...
    "Set via EXTEND.md account config, environment variables, or .ai-ggbond-skills/.env file." +
    partialHint
  );
}

export function loadCredentials(account?: ResolvedAccount): LoadedCredentials {
  const cwdEnvPath = path.join(process.cwd(), ".ai-ggbond-skills", ".env");
  const homeEnvPath = path.join(os.homedir(), ".ai-ggbond-skills", ".env");
  ...
  if (prefix) {
    ...
    sources.push(
      ...
      buildCredentialSource(`<cwd>/.ai-ggbond-skills/.env (${prefixedKeyLabel})`, cwdEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
      buildCredentialSource(`~/.ai-ggbond-skills/.env (${prefixedKeyLabel})`, homeEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
    );
  }

  sources.push(
    buildCredentialSource("process.env", process.env, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("<cwd>/.ai-ggbond-skills/.env", cwdEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("~/.ai-ggbond-skills/.env", homeEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
  );
