...
2. `<cwd>/.ai-ggbond-skills/.env` with `WECHAT_APP_ID=...`
3. `$HOME/.ai-ggbond-skills/.env` with `WECHAT_APP_ID=...`
...
Where to save?
A) Project-level: .ai-ggbond-skills/.env (this project only)
B) User-level: ~/.ai-ggbond-skills/.env (all projects)
```
